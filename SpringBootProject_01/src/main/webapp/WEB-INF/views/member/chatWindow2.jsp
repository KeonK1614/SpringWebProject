...
function sendMessage() {
    var message = document.getElementById("message").value; // 메시지 내용 가져오기
    var chatId = document.getElementById("chatId").value; // 사용자 ID 가져오기	
    var roomname = document.getElementById("roomname").value; // roomname 가져오기	

    const messageRef = ref(db, "Messages/" + roomname + "/");
    push(messageRef).then((newRef) => {
        return set(newRef, {
            sender: chatId,
            roomname: chatId,
            message: message
...
<body> 
    ...
    <input type="hidden" id="roomname" value="${Id }chatroom"  />
	...
	<input type="button"  id="enterBtn" value="전송">
